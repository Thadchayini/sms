...
<?php
    ...
    $request=mysqli_query($con,"SELECT `grade_name`, `grade_index`, `grade_color` FROM `grades` where grade_id=$grade") or die(mysqli_error($con));
...
<html>
    ...
    <body>
       ...
       <div class="card-header info-color white-text text-center py-4" style = "background-color:lightblue;">
        ...
        <?php
                while($row_subject=mysqli_fetch_assoc($request_student))
                {
            ?>
            <tr style="background-color:<?php echo $row['subject_color']; ?>">
                <td><?php echo $row_student["subject_name"];?></td>
            </tr>
            <?php
                }
            ?>
        <a href="index.php">Back to student table</a>
        </div>
        </div>
        </div>
    </body>
